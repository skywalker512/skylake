...
/**
 * A unique id used to store the editor state by id. Required if rendering
 * multiple `Plate`. Optional otherwise.
 *
 * @default 'main'
 */
...
> = {
  /**
   * A unique id used to store the editor state by id. Required if rendering
   * multiple `Plate`. Optional otherwise. Default is `'main'`.
   */
  id: string
...
> &
  Nullable<{
    ...
    /**
     * If true, plate will create the editor with `withPlate`. If false, plate
     * will remove the editor from the store.
     *
     * @default true
     */
    enabled: boolean
